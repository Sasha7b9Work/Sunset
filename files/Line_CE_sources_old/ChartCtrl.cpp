...
CChartCtrl::CChartCtrl()
{
	...
	EdgeType = EDGE_SUNKEN;//EDGE_RAISED;//0;//
...
{ 
	...
	for(i=0; i<m_pFamilyList.size();i++)	//!!!
		delete m_pFamilyList[i];			//!!!
	delete m_pSecant;
}
...
//My->*********************************
		...
		CPoint p;
		CPoint NearestPoint = GetNearestSeriePoint(0, point,X,Y); //Для начала берем за 0 график за ближайший
		int NearestSerieIndex = m_pMarkersList[m_iActiveMarkerIndex]->GetRelatedSerieIndex();
		
		int seriesCount = m_pSeriesList.size();
		copy_X = X;
		copy_Y = Y;
		for(int i = 0; i < seriesCount; i++) // Определяем ближайший график к текущему положению марекера
		{
			p = GetNearestSeriePoint(i, point,X,Y);	
		...
		m_pMarkersList[m_iActiveMarkerIndex]->SetRelatedSerie(NearestSerieIndex);		
		//устанавливаем позицию маркера
//		m_pMarkersList[m_iActiveMarkerIndex]->SetPosition(X,Y);
		m_pMarkersList[m_iActiveMarkerIndex]->SetPosition(copy_X,copy_Y);		
		
//		m_pMarkersList[m_iActiveMarkerIndex]->SetPosition(X,Y);
		m_pSecant->SetPoints(copy_X,copy_Y);//X,Y);
//    	RefreshCtrl();//9.06.15
...
CPoint CChartCtrl::GetNearestSeriePoint(int SerieIndex, CPoint point, double& XValue, double& YValue)
{
	return m_pSeriesList[SerieIndex]->GetNearestPoint(point, XValue, YValue);	
}
